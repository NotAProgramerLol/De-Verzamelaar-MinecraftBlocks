...
for ($i = 0; $i < count($items); $i++) {
    $checkAv = mysqli_query($dbConnection, "SELECT `ID`, `ProductAvailability` FROM `Products` WHERE `ID` = '" . $items[$i] . "'");
...
for ($i = 0; $i < count($items); $i++) {
    $result = mysqli_query($dbConnection, "INSERT INTO `Collection` (`Order`,`Product`) VALUES ('" . $userDetails["ID"] . "', '" . $items[$i] . "') WHERE `ID` = '" . $items[$i] . "'");
    if (!$result) {
    ...
    $remove = mysqli_query($dbConnection, "UPDATE `Products` SET `ProductAvailability` = '" . (intval($checkAv["ProductAvailability"]) - 1) . "' WHERE `ID` = '" . $items[$i] . "'");
}
