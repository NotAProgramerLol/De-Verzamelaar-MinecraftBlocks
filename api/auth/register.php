<?php
    ...
    //check if existg
    $searchAvName = $dbConnection->query("SELECT Email FROM Accounts WHERE Email = '$emailInput'");
    if($searchAvName->num_rows > 0) {
    ...
    while($searchNewSearchUrl == true) {
        ...
        $searchAvUrl = $dbConnection->query("SELECT `SearchableID` FROM `Accounts` WHERE `SearchableID` = '$searchUrl'");
        if($searchAvUrl->num_rows == 0) {
	...
	$dbConnection->query("INSERT INTO `Accounts` (`Email`, `Password`, `Salt`, `Role`, `SearchableID`) VALUES ('$emailInput', '$hashed', '$salt', 'Default', '$searchUrl')");
